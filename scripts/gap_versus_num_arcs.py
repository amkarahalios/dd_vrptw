...
test_set = ["SolomonPotvinBengio_LAG_NG2","SolomonPotvinBengio_LAG_NG5","SolomonPotvinBengio_LAG_NG8",
            "SolomonPotvinBengio_LP_NG2","SolomonPotvinBengio_LP_NG5","SolomonPotvinBengio_LP_NG8"]
...
lag_lp_pairs = [['SolomonPotvinBengio_LAG_NG2', 'SolomonPotvinBengio_LP_NG2'],
                ['SolomonPotvinBengio_LAG_NG5', 'SolomonPotvinBengio_LP_NG5'],
                ['SolomonPotvinBengio_LAG_NG8', 'SolomonPotvinBengio_LP_NG8']]
...
for instance in instances:
  ...
  for lag_lp_pair in lag_lp_pairs:
    lag_method = lag_lp_pair[0]
    test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lag_method)]
    if test_instance_results.empty:
      continue
    lag_best_lb = max(test_instance_results['lb'])
    lag_best_ub = min(test_instance_results['ub'])
    lag_gap = (lag_best_ub-lag_best_lb) * 100.0 / lag_best_ub
    lag_opt_gaps.append(lag_gap)

    lp_method = lag_lp_pair[1]
    test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lp_method)]
    if test_instance_results.empty:
      continue
    lp_best_lb = max(test_instance_results['lb'])
    lp_best_ub = min(test_instance_results['ub'])
    lp_gap = (lp_best_ub-lp_best_lb) * 100.0 / lp_best_ub
    lp_opt_gaps.append(lp_gap)

    arc_sizes.append(min(test_instance_results['numArcs']))

# plot gaps on each axis and label size with marker
data_dict = {'lp': lp_opt_gaps,
             'lag' : lag_opt_gaps,
             'size' : arc_sizes}
dataframe = pandas.DataFrame(data_dict)

small_dataframe = dataframe[dataframe['size'] < 1e6]
plt.scatter(small_dataframe['lag'], small_dataframe['lp'], marker='x', color='b', label='small')

small_dataframe = dataframe[dataframe['size'] >= 1e6]
plt.scatter(small_dataframe['lag'], small_dataframe['lp'], marker='o', color='m', label='large')

plt.xlabel('LAG optimality gap (%)')
plt.xlim([0,100])

plt.ylabel('LP optimality gap (%)')
plt.ylim([0,100])
