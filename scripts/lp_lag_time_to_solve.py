# Plot the difference in OPT Gap for LP and LAG vs. # arcs in initial DD
import os
import math
import tabulate
import pandas
...
logs_dirs = ["/Users/akarahal/Desktop/dd_graph_color/logs/", "/Users/akarahal/Desktop/dd_vrptw/logs/", "/Users/akarahal/Desktop/dd_vrptw/logs/"]
instances_dirs = ["/Users/akarahal/Desktop/dd_graph_color/instances/GC/", "/Users/akarahal/Desktop/dd_vrptw/instances/Vrp-Set-Solomon/", "/Users/akarahal/Desktop/dd_vrptw/instances/Vrp-Set-HG/"]
test_sets = [["col_elim_gc_lp_0_0_MIP_3600", "col_elim_gc_lag_0_0_MIP_3600"],
             ["Vrp-Set-Solomon_LP_NG2", "Vrp-Set-Solomon_LAG_NG2"],
             ["Vrp-Set-HG_LP_NG2", "Vrp-Set-HG_LAG_NG2"]]

all_instances = []
for instance_dir in instances_dirs:
  instances = []
...
for test_set in test_sets:
  ...
  for test in test_set:
    for instance in all_instances[index]:
      log_file_name = logs_dirs[index] + test + '/' + instance + '.log'
      ...
      for line in log_file:
        ...
        if colelim_match:
          ...
          time = int(colelim_match.group(12))
...
for test_set in test_sets:
  ...
  if index <= 1:
    ...
    lag_times = []
    arc_sizes = []
  lag_only_solve_count = 0
  lp_only_solve_count = 0
  for instance in all_instances[index]:
    lp_method = test_set[0]
    lp_test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lp_method)]
    if lp_test_instance_results.empty:
      continue
    lp_best_lb = max(lp_test_instance_results['lb'])
    lp_best_ub = min(lp_test_instance_results['ub'])
    lp_solve = True
    if lp_best_lb != lp_best_ub:
      lp_solve = False
 
    lag_method = test_set[1]
    lag_test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lag_method)]
    if lag_test_instance_results.empty:
      continue
    lag_best_lb = max(lag_test_instance_results['lb'])
    lag_best_ub = min(lag_test_instance_results['ub'])
    lag_solve = True
    if lag_best_lb != lag_best_ub:
      lag_solve = False

    if lag_solve and lp_solve:
      lp_time = max(lp_test_instance_results['time'])
      if lp_time == 0:
        lp_time = 1
      lp_times.append(lp_time)

      lag_time = max(lag_test_instance_results['time'])
      if lag_time == 0:
        lag_time = 1
      lag_times.append(lag_time)
   
      arc_sizes.append(min(lp_test_instance_results['numArcs']))
    elif lag_solve:
      lag_only_solve_count += 1
    elif lp_solve:
      lp_only_solve_count += 1

  # plot gaps on each axis and label size with marker
  # plot time to solve on each axis
  if (index == 0) or (index == 2):
    print(lp_times)
    print(lag_times)
    diffs = [lp - lag for (lp,lag) in zip(lp_times,lag_times)]
    diff_avg = sum(diffs) / len(diffs)
    print(f'average diff {diff_avg}')
    print(f'lp only solve count {lp_only_solve_count}')
