...
test_set = ["Vrp-Set-Solomon_LAG_NG2", "Vrp-Set-Solomon_variable_fix_LAG_N",
            "Vrp-Set-Solomon_LP_NG2", "Vrp-Set-Solomon_variable_fix_LP_N"]

instances = []
instance_dir = "/Users/akarahal/Desktop/dd_vrptw/instances/Vrp-Set-Solomon/"
...
for test in test_set:
  for instance in instances:
    ...
    if col_elim:
      ...
      result = {'instance': instance, 'test': test, 'iterations' : numpy.nan, 'numSep' : numpy.nan, 'lb' : numpy.nan, 'numArcs': numpy.nan, 'numFixed': numpy.nan, 'time' : numpy.nan}
      results.append(result)
...
method_pairs = [["Vrp-Set-Solomon_LAG_NG2", "Vrp-Set-Solomon_variable_fix_LAG_N"],
                ["Vrp-Set-Solomon_LP_NG2", "Vrp-Set-Solomon_variable_fix_LP_N"]]
...
for method_pair in method_pairs:
  ...
  no_fix_times = []
  for instance in instances:
    fix_method = method_pair[0]
    fix_test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == fix_method)]
    if fix_test_instance_results.empty:
      continue
    fix_best_lb = max(fix_test_instance_results['lb'])
    fix_best_ub = min(fix_test_instance_results['ub'])
    if fix_best_lb != fix_best_ub:
      continue

    no_fix_method = method_pair[1]
    no_fix_test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == no_fix_method)]
    if no_fix_test_instance_results.empty:
      continue
    no_fix_best_lb = max(no_fix_test_instance_results['lb'])
    no_fix_best_ub = min(no_fix_test_instance_results['ub'])
    if no_fix_best_lb != no_fix_best_ub:
      continue
 
    fix_time = max(fix_test_instance_results['time'])
    fix_times.append(fix_time)

    no_fix_time = max(no_fix_test_instance_results['time'])
    no_fix_times.append(no_fix_time)
