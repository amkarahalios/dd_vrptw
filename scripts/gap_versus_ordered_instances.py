# Plot the difference in OPT Gap for LP and LAG vs. instances ordered
import os
import math
import tabulate
import pandas
...
test_set = ["col_elim_sop_lag_ng_2_50_N_3600",
  ...
  for instance in instances:
    ...
    for line in log_file:
      ...
      if colelim_match:
        ...
        time_result = {'instance': instance, 'test': test, 'iterations' : lagIterations, 'numSep' : numSeparations, 'lb' : lb, 'ub' : ub, 'size': size, 'time' : time}
        time_results.append(time_result)
 
    if col_elim:
      result = {'instance': instance, 'test': test, 'iterations' : lagIterations, 'numSep' : numSeparations, 'lb' : lb, 'size': size, 'time' : time}
...
lag_compare_pairs = [["col_elim_sop_lag_ng_2_50_N_3600", "col_elim_sop_lag_ng_8_50_N_3600"]]

# gather initial dd sizes and opt gap diffs
instance_opt_gap_diffs = []
ng2_lag_iters = {}
ng8_lag_iters = {}
for instance in instances:
  optimal = instance_upper_bounds[instance]
  for lag_compare_pair in lag_compare_pairs:
    lag1_method = lag_compare_pair[0]
    test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lag1_method)]
    if test_instance_results.empty:
      continue
    lag1_best_lb = max(test_instance_results['lb'])
    lag1_best_ub = min(test_instance_results['ub'])
    lag1_gap = (lag1_best_ub-lag1_best_lb) * 100.0 / lag1_best_ub
    
    lag1_iters = max(test_instance_results['iterations'])
    ng2_lag_iters[instance] = lag1_iters

    lag2_method = lag_compare_pair[1]
    test_instance_results = time_results_df[(time_results_df['instance'] == instance) & (time_results_df['test'] == lag2_method)]
    if test_instance_results.empty:
      continue
    lag2_best_lb = max(test_instance_results['lb'])
    lag2_best_ub = min(test_instance_results['ub'])
    lag2_gap = (lag2_best_ub-lag2_best_lb) * 100.0 / lag2_best_ub
 
    lag2_iters = max(test_instance_results['iterations'])
    ng8_lag_iters[instance] = lag2_iters

    instance_opt_gap_diffs.append((instance, lag1_gap - lag2_gap))

print(ng2_lag_iters)
print(ng8_lag_iters)

# order instances by gap
sorted_opt_gaps = sorted(instance_opt_gap_diffs, key=lambda x: x[1])
gaps = [x[1] for x in sorted_opt_gaps]
instances = [x[0] for x in sorted_opt_gaps]
tick_values = list(range(len(gaps)))
sizes = [instance_sizes[instance] for instance in instances]
percent_precedences = [instance_percent_precedence[instance] for instance in instances]
ng2_its = [ng2_lag_iters[instance] for instance in instances]
ng8_its = [ng8_lag_iters[instance] for instance in instances]
diff_its = [ng2 - ng8 for (ng2,ng8) in zip(ng2_its,ng8_its)]
print(diff_its)

data_dict = {'index': tick_values,
             'instance': instances,
             'size': sizes,
             'percent': percent_precedences,
             'gap': gaps,
             'ng2_its': ng2_its,
             'ng8_its': ng8_its}
...
# plot
small_dataframe = dataframe[dataframe['percent'] < 10]
print(f"avg size for small: {small_dataframe['size'].mean()}")
print(f"ng2_its: {small_dataframe['ng2_its'].median()}")
print(f"ng8_its: {small_dataframe['ng8_its'].median()}")
plt.scatter(small_dataframe['index'], small_dataframe['gap'], marker='x', color='b', label='least precedences')

medium_dataframe = dataframe[(dataframe['percent'] >= 10) & (dataframe['percent'] < 40)]
plt.scatter(medium_dataframe['index'], medium_dataframe['gap'], marker='o', color='r', label='average precedences')

large_dataframe = dataframe[dataframe['percent'] >= 40]
print(f"avg size for large: {large_dataframe['size'].mean()}")
print(f"ng2_its: {large_dataframe['ng2_its'].median()}")
print(f"ng8_its: {large_dataframe['ng8_its'].median()}")
plt.scatter(large_dataframe['index'], large_dataframe['gap'], marker='.', color='g', label='most precedences')

plt.xlabel('instances')
plt.axhline(y=0)
empty_labels = [''] * len(instances)
plt.xticks(tick_values, labels=empty_labels)

plt.ylabel('LAG2 - LAG8 (%)')
plt.ylim([-100,100])
