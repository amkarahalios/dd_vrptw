...
bool VRPTWDecisionDiagram::checkC1101SolutionPossible()
{
  std::vector<std::vector<int>> routesByLocation;
  routesByLocation.push_back({6,268,980,210,574,118,897,202,547,0});
  routesByLocation.push_back({28,775,973,188,527,245,793,636,147,747,0});
  routesByLocation.push_back({35,825,932,868,154,370,827,832,270,171,146,0});
  routesByLocation.push_back({38,633,262,63,618,632,366,192,12,929,0});
  routesByLocation.push_back({54,483,531,607,177,195,769,135,724,379,525,0});
  routesByLocation.push_back({66,545,778,129,8,491,821,150,383,695,297,0});
  routesByLocation.push_back({76,631,467,333,639,229,730,88,659,110,42,0});
  routesByLocation.push_back({79,664,49,489,490,16,669,731,375,701,436,387,0});
  routesByLocation.push_back({95,834,557,381,271,215,630,80,236,264,750,642,310,0});
  routesByLocation.push_back({106,158,81,385,537,124,816,412,160,0});
  routesByLocation.push_back({108,182,470,93,203,716,706,875,693,762,601,232,0});
  routesByLocation.push_back({115,962,486,743,440,512,397,939,123,0});
  routesByLocation.push_back({116,813,840,463,586,906,541,886,174,522,0});
  routesByLocation.push_back({122,705,316,678,461,47,773,389,603,0});
  routesByLocation.push_back({130,725,612,904,855,13,78,794,971,628,376,817,0});
  routesByLocation.push_back({164,323,242,419,179,622,450,540,0});
  routesByLocation.push_back({167,293,542,928,952,619,446,15,73,0});
  routesByLocation.push_back({175,144,373,692,133,913,539,891,892,507,0});
  routesByLocation.push_back({183,36,663,714,561,498,438,22,311,893,266,0});
  routesByLocation.push_back({184,139,955,14,645,754,700,889,791,0});
  routesByLocation.push_back({190,842,50,675,822,459,830,872,903,0});
  routesByLocation.push_back({194,84,867,779,908,595,468,847,916,0});
  routesByLocation.push_back({208,425,149,332,345,936,739,409,0});
  routesByLocation.push_back({212,859,324,421,369,261,838,697,814,0});
  routesByLocation.push_back({231,1,70,496,166,515,307,687,676,0});
  routesByLocation.push_back({238,597,287,772,506,626,471,260,0});
  routesByLocation.push_back({285,356,472,393,967,474,688,315,518,946,0});
  routesByLocation.push_back({299,449,602,546,759,377,296,995,0});
  routesByLocation.push_back({301,777,564,320,360,763,641,878,0});
  routesByLocation.push_back({306,294,504,766,343,930,756,241,230,169,0});
  routesByLocation.push_back({309,890,11,721,253,651,396,733,741,469,0});
  routesByLocation.push_back({328,92,200,846,634,624,201,445,517,454,0});
  routesByLocation.push_back({336,508,254,152,112,157,29,757,101,392,453,860,283,221,0});
  routesByLocation.push_back({351,127,682,475,235,596,749,429,0});
  routesByLocation.push_back({363,797,45,68,575,295,180,302,187,19,0});
  routesByLocation.push_back({364,492,219,170,240,216,349,178,237,176,0});
  routesByLocation.push_back({380,44,4,17,9,89,18,105,97,919,666,0});
  routesByLocation.push_back({382,784,708,460,852,428,907,0});
  routesByLocation.push_back({384,853,432,727,288,835,858,361,839,608,994,447,0});
  routesByLocation.push_back({386,792,901,909,259,319,145,803,165,128,691,668,874,0});
  routesByLocation.push_back({388,760,272,321,353,983,499,405,548,0});
  routesByLocation.push_back({394,441,40,282,298,119,610,23,524,0});
  routesByLocation.push_back({406,820,502,222,927,509,465,196,674,871,662,0});
  routesByLocation.push_back({411,563,986,938,818,910,27,189,680,153,0});
  routesByLocation.push_back({415,322,627,462,698,559,172,569,0});
  routesByLocation.push_back({435,250,629,829,523,117,768,657,863,330,248,826,0});
  routesByLocation.push_back({451,605,578,346,443,7,206,427,67,685,0});
  routesByLocation.push_back({473,109,77,417,424,90,395,519,728,65,186,0});
  routesByLocation.push_back({482,625,990,584,60,809,993,864,790,280,43,0});
  routesByLocation.push_back({487,255,899,289,572,950,423,85,554,233,881,650,647,0});
  routesByLocation.push_back({488,800,211,359,442,804,715,888,923,355,0});
  routesByLocation.push_back({532,33,209,709,991,372,894,672,805,0});
  routesByLocation.push_back({535,303,226,671,978,851,134,358,0});
  routesByLocation.push_back({536,257,121,966,635,247,113,452,448,367,347,83,500,0});
  routesByLocation.push_back({544,278,511,308,191,712,898,979,915,732,0});
  routesByLocation.push_back({562,148,291,57,131,593,953,560,418,159,513,374,0});
  routesByLocation.push_back({570,350,882,400,752,269,865,75,713,185,0});
  routesByLocation.push_back({571,681,948,91,96,378,275,341,431,606,0});
  routesByLocation.push_back({577,533,329,249,945,20,46,689,138,905,225,0});
  routesByLocation.push_back({581,933,52,335,368,654,918,530,444,699,59,140,0});
  routesByLocation.push_back({582,479,437,719,204,37,292,696,205,746,514,0});
  routesByLocation.push_back({599,755,704,99,10,649,556,598,273,717,0});
  routesByLocation.push_back({611,940,534,738,807,314,770,617,0});
  routesByLocation.push_back({615,667,808,926,964,988,277,286,824,0});
  routesByLocation.push_back({616,313,481,74,876,795,114,338,895,416,284,234,0});
  routesByLocation.push_back({623,734,228,107,776,162,992,583,0});
  routesByLocation.push_back({638,72,573,337,613,947,588,439,751,0});
  routesByLocation.push_back({648,949,197,767,214,198,64,589,0});
  routesByLocation.push_back({652,823,258,317,958,505,142,516,0});
  routesByLocation.push_back({655,976,458,798,51,591,141,239,220,55,883,637,848,0});
  routesByLocation.push_back({660,3,402,456,565,193,670,646,263,207,0});
  routesByLocation.push_back({711,403,970,729,413,866,887,408,690,812,614,845,0});
  routesByLocation.push_back({722,718,326,334,100,414,430,802,550,0});
  routesByLocation.push_back({764,879,501,683,344,426,799,594,503,399,98,82,156,0});
  routesByLocation.push_back({780,529,300,965,740,677,921,989,917,0});
  routesByLocation.push_back({785,267,837,104,132,925,526,223,24,217,568,0});
  routesByLocation.push_back({789,553,103,331,972,974,422,944,401,579,538,0});
  routesByLocation.push_back({815,658,656,771,761,168,592,464,281,391,0});
  routesByLocation.push_back({819,999,246,943,723,920,227,850,828,433,567,161,71,862,0});
  routesByLocation.push_back({836,69,694,796,951,495,420,566,21,0});
  routesByLocation.push_back({849,476,788,528,494,354,555,342,558,549,552,0});
  routesByLocation.push_back({854,783,640,861,339,457,748,325,673,735,0});
  routesByLocation.push_back({856,758,774,478,410,5,477,398,0});
  routesByLocation.push_back({902,543,679,604,742,781,880,703,493,0});
  routesByLocation.push_back({914,362,621,969,213,251,455,252,86,485,0});
  routesByLocation.push_back({922,736,34,912,61,744,911,811,0});
  routesByLocation.push_back({924,155,224,590,312,702,199,737,873,576,0});
  routesByLocation.push_back({931,173,151,137,998,163,26,665,340,0});
  routesByLocation.push_back({937,181,407,102,934,720,126,265,2,53,62,0});
  routesByLocation.push_back({941,32,520,357,686,243,327,348,0});
  routesByLocation.push_back({957,787,806,710,318,831,961,587,58,290,0});
  routesByLocation.push_back({960,810,620,959,841,580,304,480,684,833,56,0});
  routesByLocation.push_back({963,726,801,987,466,279,31,276,41,0});
  routesByLocation.push_back({968,745,218,371,643,143,844,956,661,0});
  routesByLocation.push_back({977,390,352,600,870,786,274,869,0});
  routesByLocation.push_back({981,125,954,404,39,782,256,857,510,0});
  routesByLocation.push_back({982,707,111,551,94,843,942,885,609,877,753,0});
  routesByLocation.push_back({984,896,244,497,48,935,484,653,975,434,765,0});
  routesByLocation.push_back({985,884,305,30,136,900,644,25,1000,0});
  routesByLocation.push_back({997,87,585,365,120,521,996,0});
  for (int routeIndex=0; routeIndex<routesByLocation.size(); ++routeIndex)
  {
    std::vector<int> updatedRouteArcs;
    if (!doesRouteExistByLocations(routesByLocation[routeIndex], updatedRouteArcs))
    {
      std::cout << "ERROR C1_10_1: failed at index: " << routeIndex << std::endl;
      return false;
    }
  }
